...
Bc. Ondřeji Altmanovi za trpělivost, cenné rady a vstřícnost při implementaci praktické části
...
%
\chapter{Úvod}
V současné době internetu tvoří data významnou a důležitou součást každodenního života. Problematikou dat je nejen efektivně ukládat, 
ale také je umět rychle a efektivně interpretovat. 
...
Pro snadné integrování vizualizací do aplikací třetích stran se využívá koncept embedded analytics – ten umožňuje uživatelům rychlý a efektivní přístup datům bez potřeby přecházet mezi různými aplikacemi. 
...
Velký důraz bude kladen na bezpečný přístup k datům skrze OAuth 2 protokol, který je klíčovým prvkem Emplifi Public API. Součástí bude také administrační rozhraní, jenž umožní snadné generování a obnovování OAuth 2 tokenu.
...
Na obrázku \ref{fig:emplifiDashUkazka} vidíme příklad dashboardu - skládá se z několika grafů často nazývané jako widgety \cite{emplifiDashboard}, jejichž obsah lze modifikovat pomocí přepínačů v liště. Obsah může být modifikován v časovém období (posledních 30 dní, poslední rok, konkrétní časový rozsah, ...), ale lze ho modifikovat také na základě metrik jako je např. sociální síť (Facebook, Instragram, LinkedIn), druh interakcí (komentáře, liky, sdílení) apod.
...
Existuje hned několik způsobů, jak data embedovat. Mezi nejpopulárnější iframe a HTML, webová komponenta či React SDK s voláním API. Zatímco nejjednodušší a nejrychlejším metoda je
používáním HTML iframu, pokročilejší techniky embedování využívají React SDK a API.
