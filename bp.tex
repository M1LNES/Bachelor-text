...
\begin{center}
	...
	\endlastfoot
	%
	\verb"docs" & Obsahuje složku s uživatelskou dokumentací generovanou Docosaurem. \\
	\midrule
...
nastavená hodnota tokenu a URL (\texttt{PACKAGE\_URL}), odkud knihovny stáhnout. Pokud hodnoty nastavené nejsou, automaticky bude brát hodnoty z .env filu. Následně pomocí CURL pošle dotaz na
...
Token je možné specifikovat dvěma způsoby - a to buď specifikováním v .env souboru či systémové proměnné, nebo předání v hlavičce requestu. To umožňuje větší použitelnost pro koncového
...
ukázce \ref{autorHeader}. Klientské ID a secret musí být uložené v systémové proměnné (nebo .env souboru), aby bylo možné autentizaci provést.
...
manuálně vložit do .env souboru. K tokenům je možné stále přistupovat i přes local storage, ale tato varianta je méně uživatelsky přívětivější než jedno stisknutí tlačítka.
...
Při implementaci knihovny byly vytvářeny Markdown (.md) soubory, které popisují důležité části knihovny a aplikace. Následně nástroj Docosaurus tyto soubory zkompiloval,
...
ale také pro vývojáře. Zdrojové soubory výsledné dokumentace se nachází ve složce \texttt{docs} uložené ve zdrojovém kódu knihovny.

\chapter{Testování}
\section{Testovací strategie a výsledky}
...
\section{Testovací scénáře - knihovna}
budou dopsány
\section{Testovací scénáře - uživatelské rozhraní}
budou dopsány
